  ...
  const performSave = (targetStatus) => {
    ...
    setTimeout(() => {
      if (usePrevious && previousSnap) {
        ...
        addSnapshot(thread.id, { lastAction, nextStep: nextOrBlocker, blocker: null, fileAttachment }, targetStatus);
      }
      setSaving(false);
      onClose();
    }, 100);
                ...
                {window.electronAPI && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <button
                      ...
                      className="btn-ghost"
                      style={{ height: 30, fontSize: 13, padding: '0 12px' }}
                    >
                      <Paperclip size={14} /> {fileAttachment ? 'Change Attachment' : 'Attach File'}
                    </button>
                    {fileAttachment && (
                      <span style={{ fontSize: 12, color: 'hsl(var(--text-meta))', display: 'flex', alignItems: 'center', gap: 6 }}>
