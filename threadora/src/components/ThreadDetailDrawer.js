...
import { X, ArrowRight, Ban, Play, Edit2, Copy, Check, Trash2, CheckCircle2, Pause, Clock, Paperclip, Search } from 'lucide-react';
...
export function ThreadDetailDrawer({ thread: initialThread, onClose }) {
  const { updateThreadStatus, closeThread, addContextNote, editSnapshot, deleteSnapshot, updateThreadTrack, customTracks } = useTaskStore();
  ...
  const [trackDraft, setTrackDraft] = useState(thread.track);
  const [noteQuery, setNoteQuery] = useState('');
  const allSnapshots = useTaskStore(s => s.snapshots);

  useEffect(() => {
    setTrackDraft(thread.track);
  }, [thread.id, thread.track]);
                  ...
                  {editingTrack ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <select
                        ...
                        style={{ fontSize: 11, background: 'hsl(var(--surface))', color: 'hsl(var(--text-title))', border: '1px solid hsl(var(--border))', borderRadius: 4 }}
                      >
                        {customTracks?.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                      <button onClick={() => { updateThreadTrack(thread.id, trackDraft); setEditingTrack(false); }} className="btn-ghost" style={{ height: 20, fontSize: 10, padding: '0 4px' }}>Save</button>
                    </div>
                  ) : (
