...
import { Clock, ArrowRight, Ban, Play, Paperclip } from 'lucide-react';
...
export function ResumeCard({ thread, onStartWorking }) {
  ...
  const handleAddNote = () => {
    if (note.trim() || fileAttachment) {
      const result = addContextNote(thread.id, note, thread.status === 'paused' ? 'Added while paused' : 'Added while stuck', fileAttachment);
